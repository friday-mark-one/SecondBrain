...
function appendLedger(item, destination, now) {
...
function fileItem({ destination, item, type, category, hint, now }) {
  ...
  appendLedger(item, destination, now);
...
if (require.main === module) {
  ...
  try {
    if (cmd === "routes") {
      console.log(JSON.stringify(scanRoutes(), null, 2));
    } else if (cmd === "inbox-take") {
      console.log(JSON.stringify(inboxTake()));
    } else if (cmd === "file") {
