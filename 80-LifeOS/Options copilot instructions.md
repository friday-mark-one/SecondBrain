...
**Macro calendar (informational ONLY — never a gate)**: `live/macro_calendar.csv` holds scheduled FOMC / CPI / jobs-report dates. The pulse sends a Telegram heads-up when one is exactly `macro_alert_days` (config, default 2) trading days out, plus a once-a-month note in April/September; reports and tickets show upcoming events within 5 trading days; entry snapshots silently record event proximity for future audits. **Phase 13 (archive) tested and REJECTED macro days as a blackout — they move mega-caps no more than ordinary days — so these lines are context for the human, never a refusal, never scored in the TLDR. Do not present them as warnings against entering.** The Sunday digest warns when the calendar file has <30 days of future coverage — refresh it by asking Claude (FOMC dates: federalreserve.gov; CPI/jobs schedules: bls.gov).
