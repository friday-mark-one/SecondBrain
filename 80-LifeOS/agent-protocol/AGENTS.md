...
- Directory: `~/SecondBrain/80-LifeOS/`
- Standard: Treat this vault as your singular source of truth
- For anything related to meal plan, grocery shopping, generic item shopping or inventory refer `~/SecondBrain/80-LifeOS/Meal plan instructions.md` for complete instructions before making any edits or fetching any data.
- Writing: To capture a thought, list item, todo, or note, follow `~/SecondBrain/80-LifeOS/Capture instructions.md` (route via `capture.js`). For workouts or anything time-series, append to a dated log. Never hand-edit a destination note's items when capture.js can do it.
...
- Logging: Keep a log of everything with mm-dd-yy.md format in the `~/SecondBrain/80-LifeOS/logs` directory. If the file exists, append to it.
